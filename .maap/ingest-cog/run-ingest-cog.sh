...
# `conda activate` normally sets these via activation scripts; we
# bypass activation so do it ourselves.
...
# Put the env's bin on PATH so CLI tools (gdalinfo, gdal_translate,
# etc.) used via subprocess.run() inside our Python can find them.
